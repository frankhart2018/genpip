...
def add_dependencies(setup_file_contents, setup_file_path):
    ...
    setup_file_lines = setup_file_lines[:-1] + [install_requires_statement] + setup_file_lines[-1:]
...
def deploy(mock=False):
    ...
    if not "install_requires" in setup_file_contents:
        add_dependencies(setup_file_contents=setup_file_contents, setup_file_path=setup_file_path)
